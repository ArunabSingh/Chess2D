...
public class Pawn extends Piece {
	...
	public Pawn(final boolean black) {
		this.black = black;
		if (black) {
			image = this.getClass().getResource("/images/bPawn.png").getPath();
		} else {
			image = this.getClass().getResource("/images/wPawn.png").getPath();
		}
	}
		...
		if (checkIfPathClear(currentPos, moveTo, 0)) {
			if (canCharge(currentPos)) {
				if (moveTo[1] == currentPos[1]) {
					if (moveTo[0] - currentPos[0] >= -2 && moveTo[0] - currentPos[0] < 0 && !black) {
						return true;
					} else if (moveTo[0] - currentPos[0] <= 2 && moveTo[0] - currentPos[0] > 0 && black) {
						return true;
					} else {
						return false;
					}
