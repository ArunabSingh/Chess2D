...
public abstract class Piece {
	...
	public abstract boolean isValidKill(String currentPos, String moveTo, String color);

	public abstract String getColor();

	Square[][] squares = Board.getSquare();
	...
	protected boolean checkInDirectionStraight(int numOfSquares, int vector, int direction, int[] currentPos,
			boolean diagnal) {
		...
		if (diagnal) {
			if (direction != 0) {
				for (int i = 1; i < numOfSquares; i++) {
					...
					System.out
							.println(i + "||| checking square; " + (xaxis) + ", " + (yaxis) + ", VECTROE: " + vector);
			...
			for (int i = 1; i < numOfSquares; i++) {
				xaxis = currentPos[0] + (i * vector);
				yaxis = currentPos[1];
				System.out.println(i + "||| checking square; " + (xaxis) + ", " + (yaxis) + ", VECTROE: " + vector);
				if (squares[xaxis][yaxis].getPiece() != null) {
					System.out.println("is NOT NULL");
					return false;
				}
